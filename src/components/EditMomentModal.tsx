...
export default function EditMomentModal({ isOpen, onClose, moment }: EditMomentModalProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      if (momentImages.length >= 3) {
        setErrorStatus('Limit reached: Max 3 photos per page.');
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorStatus('Failed to update page. Connection might be weak.');
    } finally {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            <div className="flex h-16 items-center justify-between border-b border-natural-border px-8">
              <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-natural-text">Edit Page</h2>
              <button 
                onClick={onClose}
