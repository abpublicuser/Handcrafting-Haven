...
export default function EditAlbumModal({ isOpen, onClose, album }: EditAlbumModalProps) {
  const [name, setName] = useState(album.name);
  const [preview, setPreview] = useState<string | null>(album.coverImageUrl);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setPreview(album.coverImageUrl);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorStatus('Failed to update album. Please try again.');
    } finally {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            <div className="flex h-16 items-center justify-between border-b border-natural-border px-8">
              <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-natural-text">Edit Album</h2>
              <button 
                onClick={onClose}
